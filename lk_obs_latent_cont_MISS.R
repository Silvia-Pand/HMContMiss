lk_obs_latent_cont_MISS <- function(th,Y,R,yv,XXdis,Xlab,ZZdis,Zlab,param,fort=TRUE,drop=FALSE){
...
# preliminaries
  if(is.null(R)) miss = FALSE else miss = any(!R)
  	...
  	if(param=="multilogit"){
  		if(drop) nc2 = dim(ZZdis)[2]/((k+1)-1)-1
  		else nc2 = dim(ZZdis)[2]/(k-1)-1
...
# parameters on transition probabilities
    if(param=="multilogit"){
    	  if(drop){
    	    ind = count+(1:((nc2+1)*((k+1)-1)*k))
          Ga = matrix(th[ind],(nc2+1)*((k+1)-1),k)
		...
		if(drop) count = count+(nc2+1)*((k+1)-1)*k
		else count = count+(nc2+1)*(k-1)*k
   	...
   	iSi = solve(Si)
   	Yv = matrix(Y,n*TT,r)
   	if(drop) Vv = matrix(aperm(V,c(1,3,2)),n*TT,k+1)
   	else Vv = matrix(aperm(V,c(1,3,2)),n*TT,k)
   	if(miss){
   	  Sitmp = matrix(0,r,r)
   	...
   	# Update piv and Pi
   	out = est_multilogit(V[,1:k,1],XXdis,Xlab,be,Pivdis,fort=fort,ex=TRUE)
   	sc = c(sc,out$sc)
