bootstrap.MISS <- function(Y,X1,X2,param="multilogit",Mu,Si,Be,Ga,B=100){
...
# preliminaries
  ...
  for (b in 1:B) {
    ...
    #out = est_lm_cov_latent_cont(Yb,X1b,X2b,param=param,k=k)
    out <- lmcovlatent.cont.MISS(Y=Yb,X1=X1b,X2=X2b,k=k,param=param,output=TRUE)
